...
cor.test(rip_all$ppt, rip_all$vpd_max) # 0.26 correlation for both areas.
# model selection! ~
...
AIC(lmer(evi_mean ~ ppt + (1 | name), data = rip_all))
r.squaredGLMM(lmer(evi_mean ~ ppt + (1 | name), data = rip_all))
AIC(lmer(evi_mean ~ vpd_max + (1 | name), data = rip_all))
r.squaredGLMM(lmer(evi_mean ~ vpd_max + (1 | name), data = rip_all))

AIC(lmer(evi_mean ~ vpd_max*ppt + (1 | name), data = rip_all))
r.squaredGLMM(lmer(evi_mean ~ vpd_max*ppt + (1 | name), data = rip_all))

AIC(lmer(evi_mean ~ szn*ppt + (1 | name), data = rip_all))
r.squaredGLMM(lmer(evi_mean ~ szn*ppt + (1 | name), data = rip_all))


AIC(lmer(evi_mean ~ vpd_max*szn + (1 | name), data = rip_all))
r.squaredGLMM(lmer(evi_mean ~ vpd_max*szn + (1 | name), data = rip_all))


AIC(lmer(evi_mean ~ ppt*vpd_max*szn + (1 | name), data = rip_all))
r.squaredGLMM(lmer(evi_mean ~ ppt*vpd_max*szn + (1 | name), data = rip_all))
anova(lmer(evi_mean ~ ppt*vpd_max*szn + (1 | name), data = rip_all))

AIC(lmer(evi_mean ~ ppt*vpd_max*month + (1 | name), data = rip_all))

# Upland:

AIC(lmer(evi_mean ~ ppt + (1 | name), data = up_all))
summary(lmer(evi_mean ~ ppt + (1 | name), data = up_all))
r.squaredGLMM(lmer(evi_mean ~ ppt + (1 | name), data = up_all))

AIC(lmer(evi_mean ~ vpd_max + (1 | name), data = up_all))
r.squaredGLMM(lmer(evi_mean ~ vpd_max + (1 | name), data = up_all))

AIC(lmer(evi_mean ~ vpd_max*ppt + (1 | name), data = up_all))
r.squaredGLMM(lmer(evi_mean ~ vpd_max*ppt + (1 | name), data = up_all))

AIC(lmer(evi_mean ~ szn*ppt + (1 | name), data = up_all))
r.squaredGLMM(lmer(evi_mean ~ szn*ppt + (1 | name), data = up_all))

AIC(lmer(evi_mean ~ vpd_max*szn + (1 | name), data = up_all))
r.squaredGLMM(lmer(evi_mean ~ vpd_max*szn + (1 | name), data = up_all))

AIC(lmer(evi_mean ~ ppt*vpd_max*szn + (1 | name), data = up_all))
AIC(lmer(evi_mean ~ ppt + vpd_max + szn + ppt:szn + vpd_max:szn + ppt:vpd_max:szn + (1 | name), data = up_all))

r.squaredGLMM(lmer(evi_mean ~ ppt*vpd_max*szn + (1 | name), data = up_all))
r.squaredGLMM(lmer(evi_mean ~ ppt + vpd_max + szn + ppt:szn + vpd_max:szn + ppt:vpd_max:szn + (1 | name), data = up_all))

anova(lmer(evi_mean ~ ppt*vpd_max*szn + (1 | name), data = up_all))
summary(lmer(evi_mean ~ ppt*vpd_max*szn + (1 | name), data = up_all))


r.squaredGLMM(lmer(evi_mean ~ ppt*vpd_max*szn + (1 | name), data = up_all))


emmeans(lmer(evi_mean ~ ppt*vpd_max*szn + (1 | name), data = up_all), specs = "szn")

plot(emtrends(lmer(evi_mean ~ ppt*vpd_max*szn + (1 | name), data = up_all), var = "ppt"))
plot(emtrends(lmer(evi_mean ~ ppt*vpd_max*szn + (1 | name), data = rip_all),
        var = "ppt"))

# PPT effect sizes:
emmip(lmer(evi_mean ~ ppt*vpd_max*szn + (1 | name), data = up_all), ppt ~ vpd_max | szn, 
