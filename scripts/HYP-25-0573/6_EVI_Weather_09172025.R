...
we_rip <- filter(weather_evi, well == "Riparian") %>% 
  full_join(fab_rip)
we_up <- filter(weather_evi, well == "Upland") %>% 
  full_join(fab_up)
we_prism <- rbind(we_rip, we_up)

# get to a monthly scale:
weather_evi_monthly <- we_prism %>%
  select(prism, name, well, date, ppt, vpdmax, evi) %>% 
  mutate(month = month(date), year = year(date)) %>%
  group_by(prism, well, month, year) %>%
  summarise(evi = median(evi),
            ppt = sum(ppt),
            vpd_max = max(vpdmax))

write_csv(weather_evi_monthly, "data/Processed/ModelInputs_AggregatedtoPRISMPixel_09172025.csv")
...
ggplot(filter(monsoon_data), aes(x = (vpd_max), y = evi, group = interaction(well)))+
  theme_light(base_size = 20)+
...
# VPD models:
summary(lm(evi ~ vpd_max, data = filter(weather_evi_monthly, well == "Riparian")))
summary(lm(evi ~ vpd_max, data = filter(weather_evi_monthly, well == "Upland")))
summary(lm(evi ~ vpd_max, data = filter(monsoon_data, well == "Riparian")))
summary(lm(evi ~ vpd_max, data = filter(monsoon_data, well == "Upland")))

# Mixed models:

mixed_ppt_m <- lmer(evi ~ ppt + (1 | well), REML = F,
                    data = monsoon_data)
summary(mixed_ppt_m)
r.squaredGLMM(mixed_ppt_m) # 33.02%
# AIC(mixed_ppt_m)

mixed_vpd_m <- lmer(evi ~ vpd_max + (1 | well), REML = F,
                    data = monsoon_data) # only have vpd slope as random?
summary(mixed_vpd_m)
